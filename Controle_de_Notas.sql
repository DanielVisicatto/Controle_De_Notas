...
SELECT  --a.Nome, (dm.Nota_1 + dm.Nota_2) / 2 
a.Nome,
"Media" = CASE 
    WHEN (dm.Nota_Subs IS NULL) THEN (dm.Nota_1 + Nota_2) / 2
    WHEN (dm.Nota_Subs > Nota_1) AND (dm.Nota_1 < Nota_2) THEN (dm.Nota_Subs + Nota_2) / 2
    WHEN (dm.Nota_Subs > Nota_2) AND (dm.Nota_2 < Nota_1) THEN (dm.Nota_Subs + Nota_1) /2
    
   -- WHEN Nota_1 < Nota_Subs THEN Nota_Subs
   -- WHEN Nota_2 < Nota_Subs THEN Nota_Subs
   -- ELSE (dm.Nota_1 + dm.Nota_2) /2
END
